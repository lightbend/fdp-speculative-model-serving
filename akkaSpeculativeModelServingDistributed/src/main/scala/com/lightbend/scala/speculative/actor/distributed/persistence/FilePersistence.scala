package com.lightbend.scala.speculative.actor.distributed.persistence
...
object FilePersistence {
  ...
  def saveDataState(dataType : String, timeout: Long, models: List[String]) : Unit = {
    val output = getDataOutputStream(s"data_$dataType")
    output.writeLong(timeout)
    output.writeLong(models.size)
    models.foreach(a => output.writeUTF(a))
    output.flush()
    output.close()
  }
